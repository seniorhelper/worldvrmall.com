...
const _pb = new Map(); const pick = (a) => { if (!a || a.length < 3) return a[Math.floor(Math.random() * a.length)]; const k = a.length + '|' + String(a[0]).slice(0, 40); let bag = _pb.get(k); if (!bag || !bag.length) { bag = a.map((_, i) => i).sort(() => Math.random() - 0.5); _pb.set(k, bag); } return a[bag.pop()]; };
...
export function paxReply(text, app) {
  ...
  if (has('hunger', 'hungry people', 'starving', 'starvation', 'donate', 'donation', 'charity', 'wfp', 'world food', 'give back', 'famine')) { const d = app && app.places.find(p => p.id === 'donate' || p.id === 'out-donate'); const gdn = app && app.places.find(p => p.id === 'garden' || p.id === 'out-garden'); return { say: 'About 25,000 people, more than 10,000 of them children, die from hunger and related causes every day (United Nations). The World Food Programme fights it and won the 2020 Nobel Peace Prize for it. If it were you, what would you want the world to do? We can change this together.', places: [d, gdn].filter(Boolean), links: [['❤️ Donate at wfp.org', 'https://www.wfp.org/'], ['📄 UN source', 'https://www.un.org/en/chronicle/article/losing-25000-hunger-every-day']] }; }
  if (has('aunt win', 'aunt', 'win')) return { say: 'Aunt Win! The creator of this world has an aunt who was named Organic Farmer of the Year in Nebraska. The Sky Garden on the mall roof is a thank-you to her: feed the soil, not the plant.', places: app ? app.places.filter(p => p.id === 'garden' || p.id === 'out-garden') : [] };
  if (has('compost', 'composting')) return { say: 'Compost: roughly two parts browns (dry leaves, cardboard) to one part greens (kitchen scraps, grass). Keep it as damp as a wrung-out sponge, turn it now and then, and in a few months you have free fertilizer. No meat or dairy in the pile.' };
  if (has('mulch', 'weeds', 'weed')) return { say: 'Mulch 2 to 3 inches deep with straw, leaves or wood chips. It holds water, smothers weeds and feeds the soil as it breaks down. Keep it a little back from the stems.' };
  if (has('pests', 'pest', 'bugs', 'aphids', 'slugs')) return { say: 'Organic pest control starts with allies: flowers bring ladybugs and lacewings, and they eat aphids. Hand-pick the big ones, use row covers on young plants, rotate crops every season, and keep plants healthy. Stressed plants get eaten first.' };
  if (has('companion', 'companions', 'tomato', 'tomatoes', 'basil', 'marigold', 'marigolds')) return { say: 'Classic companions: basil beside tomatoes, marigolds almost anywhere, and the three sisters (corn, beans, squash) together. Beans feed the soil, corn is the pole, squash shades out the weeds.' };
  if (has('watering', 'irrigation', 'drip', 'rainwater', 'rain barrel', 'ro system', 'reverse osmosis')) return { say: 'Water the roots, not the leaves, and do it in the morning. Drip lines waste the least. The mall roof catches rain in its gutters, filters it through an RO unit into two tanks, and that water feeds the Sky Garden and the fountains.' };
  if (has('garden', 'gardening', 'organic', 'grow', 'plant', 'planting', 'soil', 'vegetables', 'veggies', 'farm', 'farming', 'seeds', 'harvest')) return { say: 'Organic in one breath: feed the soil with compost, mulch deep, rotate your crops, plant flowers for the good bugs, and water at the roots in the morning. Start with one pot of something you eat. Want to try it? The Sky Garden on the roof lets you plant, water and harvest.', places: app ? app.places.filter(p => p.id === 'garden' || p.id === 'out-garden') : [], chips: ['Compost', 'Pests', 'Companion plants', 'Watering'] };
...
export function initBot(app, opts = {}) {
  ...
  const acts = (list) => { if (!list.length) return; const w = document.createElement('div'); w.className = 'pax-acts'; for (const a of list) { const el = document.createElement(a.href ? 'a' : 'button'); el.textContent = a.label; if (a.href) { el.href = a.href; if (a.ext) { el.target = '_blank'; el.rel = 'noopener'; } } else { el.type = 'button'; el.onclick = a.fn; } w.appendChild(el); } log.appendChild(w); log.scrollTop = log.scrollHeight; };
  const respond = (text) => { const r = paxReply(text, app); setTimeout(() => { add('b', esc(r.say)); const list = []; for (const p of (r.places || [])) list.push({ label: (p.icon || '📍') + ' ' + p.name, fn: () => app.travel(p) }); for (const p of (r.pages || [])) list.push({ label: '📄 ' + p[0], href: p[1] }); for (const l of (r.links || [])) list.push({ label: l[0], href: l[1], ext: true }); for (const c of (r.chips || [])) list.push({ label: c, fn: () => send(c.replace(/^[^a-zA-Z]+/, '')) }); acts(list); if (r.form) humanForm(); }, 350); };
